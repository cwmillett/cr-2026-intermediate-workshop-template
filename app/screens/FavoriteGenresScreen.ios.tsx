...
export function FavoriteGenresScreen() {
    const { favoriteGenres, otherGenres, isLoading, isFavorite, toggleFavorite } = useFavoriteGenresService();
                    ...
                    {favoriteGenres.length === 0 ? (
                        ...
                        <List.ForEach onDelete={(indices) => { indices.forEach(index => toggleFavorite(favoriteGenres[index].id)); }}>
                            {favoriteGenres.map((genre) => (
                                <Label key={genre.id} title={genre.name} modifiers={[tag(genre.id), moveDisabled(true)]} />
                        ))}
                        </List.ForEach>
                    )}
                </Section>
